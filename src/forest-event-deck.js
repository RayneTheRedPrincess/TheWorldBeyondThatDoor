...
export function planForestTrainerRoster({forestTrainers,partyBaseClasses=[],unlockedSubclasses=[],forcedTrainerIds=[],rng=Math.random}={}){
  const entries=forestTrainers?.entries||[]; if(!entries.length)return {activeTrainerIds:[],anchorTrainerIds:[],scheduledByDepth:{},partyBaseClasses:[]};
  ...
  for(const baseClass of party){
    ...
    let candidates=entries.filter(t=>t.baseClass===baseClass&&!selected.includes(t.id));
  ...
  while(selected.length<desired){
    ...
    const chosen=weightedPick(candidates,t=>{
      let w=party.includes(t.baseClass)?Number(rules.matchingFamilyWeight||4):1;
      if(!unlocked.has(t.subclass))w*=1.25;
      return w;
    },rng);
  ...
  const assign=(id,preferEarly=false)=>{
    ...
    let depths=[];
    for(let d=Math.max(3,Number(t.minDepth||3));d<=Math.min(29,Number(t.maxDepth||29));d++){
      if(d===15||d===30||usedDepths.has(d))continue;
      if(preferEarly&&d>14)continue;
      if(!preferEarly&&earlyIds.includes(id)===false&&d<16)continue;
      depths.push(d);
    }
    if(!depths.length){
      for(let d=Math.max(3,Number(t.minDepth||3));d<=Math.min(29,Number(t.maxDepth||29));d++) if(d!==15&&d!==30&&!usedDepths.has(d))depths.push(d);
    }
    const depth=pickOne(depths,rng); if(depth!=null){usedDepths.add(depth);scheduledByDepth[String(depth)]=id;}
  };
  for(const id of earlyIds)assign(id,true);
  for(const id of selected)if(!earlyIds.includes(id))assign(id,false);
  return {activeTrainerIds:selected,anchorTrainerIds:anchors,scheduledByDepth,partyBaseClasses:party,rosterSize:selected.length};
}
