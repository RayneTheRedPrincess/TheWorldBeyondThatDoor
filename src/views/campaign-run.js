...
function campsiteLoadout(run,catalog,crafting,craftingUi,presentationUi={}){
 ...
 return `<div class="campsite-workspace ${sidebarOpen?'':'sidebar-collapsed'}">${sidebar}<div class="campsite-main-pane">${craftingPanel(run,crafting,catalog,craftingUi,owner?.baseClass)}</div></div>`;
}
...
function expeditionBody(run, baseAbilities, subclassAbilities, equipmentCatalog, forestCrafting, craftingUi, presentationUi = {}, contentPortraits = null) {
  ...
  if (expedition.state === 'combat-pending') {
    ...
    const source = 'The chosen route has become a battle.';
    return `
  ...
  if (expedition.state === 'noncombat-pending') {
    ...
    return `
      <div class="encounter-lock">
        ...
        <p class="muted">Resolving a stat check does not automatically start a combat. After the result, you may continue deeper normally.</p>
  ...
  if (expedition.state === 'event-result') {
    ...
    return `<div class="encounter-lock event-result-card"><div class="kicker">${escapeHtml(expedition.regionName || 'Regional')} Check Result</div><h3>${label}</h3><div class="notice"><strong>${escapeHtml(d.stat||'Check')}</strong> · Roll ${Number(d.roll||0)} + ${Number(d.modifier||0)} = ${Number(d.total||0)} vs DC ${Number(d.dc||0)}</div>${eventEffectSummary(d,forestCrafting,equipmentCatalog)}<button class="primary" data-action="forest-event-continue-combat">Continue Expedition</button></div>`;
  }

  if (expedition.state === 'campsite') {
    const finalPreparation=Boolean(expedition.campsite?.finalPreparation);
    return `
      <div class="campsite-owner">
        <div class="kicker">${finalPreparation?'Final Preparation · Full Heal':'Mandatory Post-Battle Campsite'}</div>
        <h3>${finalPreparation?'The last camp waits beneath the Shadow Infused Dark Woods.':'The party has stopped for camp.'}</h3>
        <p class="muted">${finalPreparation?'Every living real party member has been restored to full HP. There is no battle on Depth 1: adjust equipment, consumables, and crafting before confronting the cult leader on Depth 2.':'The expedition cannot advance until the campsite is completed. Equipment and combat-consumable choices made here apply to the next battle.'}</p>
