...
import { aggregateEquipmentEffects, applyEquipmentCoreStats, equipmentInventoryEntries, equipmentOwnerState, legalEquipmentSlots } from '../equipment-controller.js';
...
function actorCard(actor, currentActorId, { presentation, featured = false, targetable = false, showNumbers = true } = {}) {
  ...
  return `<article class="${classes}" data-side="${escapeHtml(actor.side)}" data-combat-actor-id="${escapeHtml(actor.id)}" ${targetable && !defeated ? 'data-action="combat-select-actor" tabindex="0" role="button" aria-label="Select ${escapeHtml(actor.name)} as target"' : ''}>
    ...
    <div class="actor-portrait-shell" aria-hidden="true"><div class="actor-portrait">${actor.portraitAsset?`<img src="${escapeHtml(actor.portraitAsset)}" alt="" draggable="false">`:`<span>${escapeHtml(initials)}</span>`}</div>${featured ? '<div class="boss-crown">✦</div>' : ''}</div>
...
function craftingPanel(run,crafting,catalog,ui={},baseClass=null){
 ...
 const rows=listCraftingRecipes(run,crafting,catalog,filterArgs),groups=new Map();for(const row of rows){const key=row.recipe.category||'Other';if(!groups.has(key))groups.set(key,[]);groups.get(key).push(row);}const categories=[...new Set((crafting?.recipes||[]).map(r=>r.category||'Other'))];
...
function combatBody(run, combat, baseAbilities, subclassAbilities, equipmentCatalog, presentationUi = {}) {
  ...
  const playerControls = !playerTurn ? '' : (actionTaken ? `<div class="combat-command-bar completed"><button data-action="combat-end-turn">End Turn</button></div>` : (current?.keptState?.perId?.['KI-184']?.awaitingChoice ? keptCombatStartPrompt(current) : (current?.classState?.baseClass === 'Druid' && !current.classState?.form ? `<div class="notice"><strong>Choose Starting Form</strong><div class="combat-actions"><button data-action="combat-druid-form" data-form="Fang">Fang</button><button data-action="combat-druid-form" data-form="Grove">Grove</button><button data-action="combat-druid-form" data-form="Bloom">Bloom</button></div></div>` : `
    ...
    <div class="combat-action-drawer">
      ${panel === 'consumable' ? combatConsumablePanel(run, combat, current, equipmentCatalog) : `${abilityPanel(combat,current,baseAbilities)}${subclassAbilityPanel(combat,current,subclassAbilities)}${equipmentAbilityPanel(combat,current)}${keptActivePanel(combat,current)}`}
    </div>`)));
  ...
  return `<div class="combat-foundation combat-presentation ${accessibility}" style="--combat-speed:${speed}" data-scene="${escapeHtml(scene)}" data-presentation-id="${escapeHtml(presentation.presentationId||'')}" data-semantic-action="${escapeHtml(presentation.semanticAction||'')}"><div class="combat-layout"><aside class="combat-left-sidebar"><div class="combat-head combat-hud-top"><div><div class="kicker">Combat · Round ${Number(combat.round||1)}</div><h3>${current?`${escapeHtml(current.name)}'s Turn`:'Battle'}</h3><div class="muted">Fixed battlefield · action-only motion</div></div><label class="combat-speed-control"><span>Combat Speed</span><select data-setting="combatSpeed" aria-label="Combat speed">${[.1,.25,.5,.75,1,1.25,1.5,1.75,2].map(v=>`<option value="${v}" ${Number(v)===speed?'selected':''}>${v}×</option>`).join('')}</select></label></div>${commandHtml}</aside><main class="combat-center-stage">${battleHtml}</main><aside class="combat-right-sidebar"><section class="combat-turn-order-panel"><div class="kicker">Turn Order</div>${turnOrderHtml}</section><details class="combat-log-panel" open><summary>Combat Log <span>${combatLog.length} recent entries</span></summary><div class="combat-log-lines">${combatLog.length?combatLog.map(line=>`<div>${escapeHtml(line)}</div>`).join(''):'<div class="muted">Combat events will appear here.</div>'}</div></details></aside></div></div>`;
}
...
export function renderCampaignRun({ run, baseAbilities, subclassAbilities, progression, equipmentCatalog, forestCrafting, forestTrainers, maraQuestStatus = null, craftingUi, presentationUi = {} }) {
  ...
  return shell(`
    <section class="campaign-run-hero panel ${run.configuration?.portraitAsset?'with-vessel-portrait':''}">
      ${run.configuration?.portraitAsset?`<div class="campaign-vessel-portrait"><img src="${escapeHtml(run.configuration.portraitAsset)}" alt="Selected Vessel portrait"></div>`:''}
