...
import { buildRecords, keptImpressionShopRequirementState, regionalRaceUnlockState } from '../tavern-services-controller.js';
import { tutorialTokenBalance, tutorialStatus, raceChoiceTokenBalance } from '../tutorial-controller.js';
...
function krassLibrary(slot, account, keptEntries, runtimeEntries, tavernAdventurers, tavernServices, message, ux={}) {
  ...
  const shopOffers=tavernServices?.keptImpressionShop?.offers||[];
  const onyx=Math.max(0,Math.trunc(Number(account.currencies?.onyx||0)));
  const shopPanel=shopOffers.length?`<section class="section boon-shop-panel"><div class="section-title"><div><div class="kicker">Region-earned Kept Impressions</div><h3>Krass's Purchasable Kept Impressions</h3><p class="muted">Selected Kept Impressions become purchasable with account Onyx after their Forest or Bog accomplishment is met. Requirements, purchases, and ownership are account-wide and never tied to one Vessel slot.</p></div><div class="library-capacity"><span>Account Onyx</span><strong>${onyx}</strong></div></div>${['forest','bog-of-lost-souls'].map(regionId=>{const offers=shopOffers.filter(offer=>(offer.region||'forest')===regionId);if(!offers.length)return '';const regionName=regionId==='bog-of-lost-souls'?'Bog of Lost Souls':'Forest';return `<div class="shop-region-group"><div class="section-title compact"><div><div class="kicker">${escapeHtml(regionName)}</div><h4>${escapeHtml(regionName)} unlocks</h4></div></div><div class="kept-grid">${offers.map(offer=>{const entry=keptEntries.find(e=>e.id===offer.keptId);if(!entry)return '';const req=keptImpressionShopRequirementState(account,offer);const already=owned.has(entry.id);const cost=Math.max(0,Math.trunc(Number(offer.onyxCost||0)));const afford=onyx>=cost;const disabled=already||!req.met||!afford;const button=already?'Already Kept':!req.met?`${req.region} Requirement Locked`:!afford?`Need ${cost} Onyx`:`Buy for ${cost} Onyx`;return `<article class="kept-card ${already?'equipped':''}"><div class="kept-card-title"><div><small>${escapeHtml(entry.id)} · ${entry.slots} slot${entry.slots===1?'':'s'}</small><strong>${escapeHtml(entry.name)}</strong></div><span>${cost} Onyx</span></div><p class="mechanic-copy">${escapeHtml(entry.mechanic||entry.canonical_text||'')}</p><div class="shop-requirement ${req.met?'met':'locked'}"><strong>${req.met?'Requirement met':`${req.region} requirement`}</strong><span>${escapeHtml(req.label)}</span><small>${Math.min(req.progress,req.target)} / ${req.target}</small></div><button class="${disabled?'secondary':'primary'}" data-action="kept-shop-buy" data-ki="${escapeHtml(entry.id)}" ${disabled?'disabled':''}>${escapeHtml(button)}</button></article>`;}).join('')}</div></div>`;}).join('')}</section>`:'';
...
function recordsRoom(slot,account,tavernAdventurers) { const r=buildRecords(account,slot,tavernAdventurers);const perf=r.notableCombat||{};return `<section class="panel room-panel"><div class="kicker">Trophy / Records Room</div><h2>Chronicle of Returns</h2><div class="identity-grid"><div><span>Campaigns</span><strong>${r.campaignsCompleted}</strong></div><div><span>Victories / Returns</span><strong>${r.victories}</strong></div><div><span>Defeats</span><strong>${r.defeats}</strong></div><div><span>Highest Forest Depth</span><strong>${r.highestForestDepth}</strong></div><div><span>Highest Bog Depth</span><strong>${r.highestBogDepth}</strong></div><div><span>Bog Cleared</span><strong>${r.bogCleared?'Yes':'No'}</strong></div><div><span>Minibosses Defeated</span><strong>${r.minibossesDefeated}</strong></div><div><span>Bosses Defeated</span><strong>${r.bossesDefeated}</strong></div><div><span>Trainers Encountered</span><strong>${r.trainersEncountered.length}</strong></div><div><span>Trainers Fought / Learned</span><strong>${r.trainersFought.length} / ${r.trainersLearnedFrom.length}</strong></div><div><span>Subclasses Discovered</span><strong>${r.subclassesDiscovered}</strong></div><div><span>Adventurers Recruited</span><strong>${r.tavernAdventurersRecruited} / ${r.tavernAdventurerTotal}</strong></div></div><section class="section"><h3>Notable Combat Records</h3><div class="run-stat-strip"><span><strong>Damage Dealt</strong> ${Number(perf.mostDamageDealt?.value||0)}</span><span><strong>Damage Taken</strong> ${Number(perf.mostDamageTaken?.value||0)}</span><span><strong>Healing Done</strong> ${Number(perf.mostHealingDone?.value||0)}</span></div></section></section>`; }
...
function raceChoicePanel(account, allRaces = []) {
  ...
  if(!locked.length)return `<section class="section starter-token-panel"><h3>Free Race Choice Token</h3><p>Every available race is already unlocked for this account.</p></section>`;
  return `<section class="section starter-token-panel"><div class="kicker">One-time account reward</div><h3>Free Race Choice Token</h3><p>You have <strong>${balance}</strong> free Race Choice token. Choose one race to permanently unlock for the whole account. The unlock is shared by all nine Vessel slots and is never stored per character.</p><div class="race-token-grid">${locked.map(race=>`<button type="button" class="secondary" data-action="race-token-redeem" data-race="${escapeHtml(race)}">Unlock ${escapeHtml(race)}</button>`).join('')}</div></section>`;
}

function regionalRaceProgressPanel(account,tavernServices,allRaces=[]){const groups=tavernServices?.regionalRaceUnlocks||{};const known=new Set(allRaces||[]);const sections=[['forest','Forest'],['bog-of-lost-souls','Bog of Lost Souls']].map(([id,name])=>{const rows=(groups[id]||[]).filter(x=>known.has(x.race)).map(entry=>{const st=regionalRaceUnlockState(account,entry,id);return `<article class="service-card ${st.unlocked?'active-service':''}"><div class="kept-card-title"><strong>${escapeHtml(st.race)}</strong><span>${st.unlocked?'Unlocked':`${Math.min(st.progress,st.target)} / ${st.target}`}</span></div><p>${escapeHtml(st.label)}</p></article>`;}).join('');return rows?`<div class="race-progress-region"><div class="kicker">${escapeHtml(name)}</div><div class="kept-grid compact">${rows}</div></div>`:'';}).join('');return sections?`<section class="section regional-race-progress"><h3>Regional Race Unlocks</h3><p class="muted">These accomplishments permanently unlock races for the entire account. Your free Race Choice token can unlock any one of them early; later achievement credit simply recognizes that the race is already owned.</p>${sections}</section>`:'';}

function vesselRooms(slot, account, baseAbilities, subclassAbilities, unlockedRaces=[], allRaces=[], baseClasses=[], portraitSystem=null, tavernServices=null, message = '', ux = {}) {
  ...
  return `<section class="panel room-panel"><div class="kicker">Vessel / Player Rooms</div><h2>${escapeHtml(c.name)}</h2>
    ...
    ${regionalRaceProgressPanel(account,tavernServices,allRaces)}
    ...
    ${(()=>{
      ...
      if(staticOptions.length){
        ...
        const offset=((Number(ux?.portraitCarouselOffset||0)%staticOptions.length)+staticOptions.length)%staticOptions.length;
        const visible=staticOptions.map((_,i)=>staticOptions[(i+offset)%staticOptions.length]).slice(0,6);
        return `<section class="section portrait-library static-portrait-library"><div class="kicker">Appearance Atelier · Static Portraits · Six-Portrait Carousel</div><h3>Choose Your Vessel Portrait</h3><p class="muted">This ${escapeHtml(c.race)} ${escapeHtml(c.baseClass)} currently has ${staticOptions.length} installed choices. Exactly six Race × Gender × Subclass portraits are kept in the chooser at once. Use the arrows to rotate the six choices, then click a portrait to select it.</p><div class="portrait-carousel" aria-label="Vessel portrait chooser"><button type="button" class="portrait-carousel-arrow" data-action="portrait-carousel-prev" aria-label="Rotate portrait choices left">‹</button><div class="portrait-choice-grid portrait-carousel-grid">${visible.map(x=>`<button type="button" class="portrait-choice ${selectedId===x.id?'selected':''}" data-action="static-portrait-select" data-portrait="${escapeHtml(x.id)}">${portraitInnerMarkup({asset:x.asset,alt:`${x.race} ${x.gender} ${x.subclass} portrait`,size:'chooser',loading:'lazy',fetchPriority:'low'})}<span>${escapeHtml(x.subclass)}</span><small>${escapeHtml(x.gender[0].toUpperCase()+x.gender.slice(1))} · ${escapeHtml(x.race)}</small></button>`).join('')}</div><button type="button" class="portrait-carousel-arrow" data-action="portrait-carousel-next" aria-label="Rotate portrait choices right">›</button></div><p class="portrait-carousel-status muted">6 visible · ${escapeHtml(c.baseClass)} family · ${escapeHtml(c.race)}</p></section>`;
      }
...
function adventurerQuarters(slot, account, tavernAdventurers, tavernServices, message='') {
  ...
  return `<section class="panel room-panel"><div class="kicker">Tavern Adventurer Quarters</div><h2>Adventurer Quarters</h2><p class="muted">Tavern Adventurers act on their own in combat according to personality, class, subclass, and role. Up to three recruited Adventurers may deploy; they never use consumables.</p>${message?`<div class="notice section">${escapeHtml(message)}</div>`:''}<div class="kept-grid section">${entries.map(a=>{const joined=recruited.has(a.id),active=selected.has(a.id);return `<article class="kept-card adventurer-card ${active?'equipped':''}">${a.portrait?`<div class="adventurer-portrait">${portraitInnerMarkup({asset:a.portrait,alt:`Portrait of ${a.name}`,size:'full',loading:'lazy'})}</div>`:''}<div class="kept-card-title"><div><small>${escapeHtml(a.personality)} · ${escapeHtml(a.combatRole)}</small><strong>${escapeHtml(a.name)}</strong></div><span>${escapeHtml(a.race || 'Unknown Race')}</span></div><p>${escapeHtml(a.baseClass)} / ${escapeHtml(a.subclass)} · ${escapeHtml(a.priority)}</p>${joined?'':`<p class="field-help">Recruitment: ${escapeHtml(recruitMap.get(a.id)||'Available from Forest accomplishments')}</p>`}<button data-action="adventurer-toggle" data-adventurer="${escapeHtml(a.id)}" ${joined?'':'disabled'}>${active?'Remove from Party':joined?'Deploy':'Not Recruited'}</button></article>`}).join('')}</div><p class="muted section">Only Tavern Adventurers who have joined this account can be deployed.</p></section>`;
}
...
function roomContent(roomId, ctx) {
  ...
  if (roomId === 'krass-library') return krassLibrary(ctx.slot, ctx.account, ctx.keptEntries, ctx.keptRuntimeEntries, ctx.tavernAdventurers, ctx.tavernServices, ctx.message,ctx.ux);
  ...
  if (roomId === 'vessel-rooms') return vesselRooms(ctx.slot, ctx.account, ctx.baseAbilities, ctx.subclassAbilities, ctx.unlockedRaces, ctx.allRaces, ctx.baseClasses, ctx.portraitSystem, ctx.tavernServices, ctx.message, ctx.ux);
...
export function renderTavern({ room, slot, account, subclassesForBase = [], unlockedRaces = [], allRaces = [], baseClasses = [], keptEntries = [], keptRuntimeEntries = [], tavernAdventurers = null, tavernServices = null, equipmentCatalog = null, baseAbilities = null, subclassAbilities = null, portraitSystem = null, message = '', ux = {} }) {
  const inMain = room.id === 'main-hall';
  return shell(`${roomContent(room.id, { slot, account, subclassesForBase, unlockedRaces, allRaces, baseClasses, keptEntries, keptRuntimeEntries, tavernAdventurers, tavernServices, equipmentCatalog, baseAbilities, subclassAbilities, portraitSystem, message, ux })}${inMain ? '' : '<div class="section"><button class="secondary" data-action="tavern-main-hall">Return to Main Hall</button></div>'}`, { back: inMain, backAction: 'leave-tavern', backLabel: 'Return Home' });
}
