...
import { portraitOptionsForBase, realisticPortraitDescriptor } from '../portrait-controller.js';
...
function vesselRooms(slot, account, baseAbilities, subclassAbilities, unlockedRaces=[], baseClasses=[], portraitSystem=null, message = '') {
  ...
  return `<section class="panel room-panel"><div class="kicker">Vessel / Player Rooms</div><h2>${escapeHtml(c.name)}</h2>
    ...
    ${message ? `<div class="notice section">${escapeHtml(message)}</div>` : ''}
    ${(()=>{
      const family=[...new Set((subclassAbilities?.abilities||[]).filter(a=>a.baseClass===c.baseClass).map(a=>a.subclass))].sort();
      const genders=portraitSystem?.vesselPortraits?.genders||[];
      const installed=[];for(const subclass of family)for(const gender of genders){const d=realisticPortraitDescriptor({race:c.race,gender,subclass},portraitSystem);if(d?.ready)installed.push(d);}
      if(!installed.length)return `<section class="section appearance-atelier unavailable"><div class="kicker">Appearance Atelier</div><h3>Realistic Portrait Customization</h3><p class="muted">The new realistic portrait system is active, but this Race × Gender × Subclass identity has not been installed yet. Compatibility portraits remain available below while the library expands.</p></section>`;
      const selectedId=c.appearance?.portraitIdentity;const selected=installed.find(x=>x.id===selectedId)||installed[0];const saved=selectedId===selected.id?(c.appearance?.portraitColors||{}):{};const colors=Object.fromEntries(selected.channels.map(ch=>[ch.id,saved[ch.id]||ch.default]));const masks=Object.fromEntries(selected.channels.map(ch=>[ch.id,ch.mask]));
      const selectorPayload=installed.map(x=>({id:x.id,label:x.label,gender:x.gender,subclass:x.subclass,asset:x.asset,channels:x.channels}));
      return `<form id="appearance-atelier-form" class="section appearance-atelier stat-panel" data-portrait-options="${escapeHtml(JSON.stringify(selectorPayload))}"><div class="stat-panel-head"><div><div class="kicker">Appearance Atelier · A3</div><h3>Realistic Vessel Portrait</h3></div><span class="portrait-system-chip" data-portrait-system-chip>${escapeHtml(selected.label)}</span></div><p class="muted">Choose any installed realistic portrait from this Vessel's current class family, then recolor its hair, skin, clothing, armor, accent and eyes. The selection is cosmetic and persists with this Vessel.</p><label class="portrait-identity-picker"><span>Installed Portrait Identity</span><select data-portrait-identity-select>${installed.map(x=>`<option value="${escapeHtml(x.id)}" ${x.id===selected.id?'selected':''}>${escapeHtml(`${x.gender[0].toUpperCase()+x.gender.slice(1)} · ${x.subclass}`)}</option>`).join('')}</select></label><input type="hidden" name="portrait_gender" value="${escapeHtml(selected.gender)}"><input type="hidden" name="portrait_subclass" value="${escapeHtml(selected.subclass)}"><div class="appearance-atelier-layout"><div class="appearance-preview-frame">${portraitInnerMarkup({asset:selected.asset,masks,colors,alt:`${selected.label} preview`})}</div><div class="appearance-controls">${selected.channels.map(ch=>`<label class="portrait-color-control"><span>${escapeHtml(ch.label)}${ch.optional?' <small>(optional)</small>':''}</span><input type="color" name="portrait_color_${escapeHtml(ch.id)}" value="${escapeHtml(colors[ch.id])}" data-portrait-color="${escapeHtml(ch.id)}" data-default="${escapeHtml(ch.default)}"></label>`).join('')}</div></div><div class="bind-actions section"><button type="button" class="secondary" data-action="portrait-colors-reset">Reset Colors</button><button type="submit" class="primary">Save Realistic Appearance</button></div></form>`;
    })()}
    ${(()=>{const portraitOptions=portraitOptionsForBase(c.baseClass,subclassAbilities);const selected=c.appearance?.portraitId||'';return `<section class="section portrait-library"><div class="kicker">Vessel Portrait Library</div><h3>Choose a Portrait</h3><p class="muted">Each of this base class’s three subclass identities has three masculine and three feminine portrait variants. Portrait choice is cosmetic and persists on this Vessel.</p><div class="portrait-choice-grid">${portraitOptions.map(x=>`<button type="button" class="portrait-choice ${selected===x.id?'selected':''}" data-action="vessel-portrait-select" data-portrait="${escapeHtml(x.id)}"><img src="${escapeHtml(x.asset)}" alt="${escapeHtml(`${x.subclass} ${x.presentation} portrait ${x.variant}`)}" loading="lazy"><span>${escapeHtml(x.subclass)}</span><small>${escapeHtml(x.presentation)} ${x.variant}</small></button>`).join('')}</div></section>`;})()}
