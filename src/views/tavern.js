...
import { portraitOptionsForBase } from '../portrait-controller.js';
...
function vesselRooms(slot, account, baseAbilities, subclassAbilities, unlockedRaces=[], baseClasses=[], message = '') {
...
function roomContent(roomId, ctx) {
  ...
  if (roomId === 'vessel-rooms') return vesselRooms(ctx.slot, ctx.account, ctx.baseAbilities, ctx.subclassAbilities, ctx.unlockedRaces, ctx.baseClasses, ctx.message);
...
export function renderTavern({ room, slot, account, subclassesForBase = [], unlockedRaces = [], baseClasses = [], keptEntries = [], keptRuntimeEntries = [], tavernAdventurers = null, tavernServices = null, equipmentCatalog = null, baseAbilities = null, subclassAbilities = null, message = '', ux = {} }) {
