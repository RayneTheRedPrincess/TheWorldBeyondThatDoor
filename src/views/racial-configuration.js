...
export function renderSelectedRaceDetailsPanels(data,{selectedRace=''}={}){
 const details=data?.raceDetails||{};const races=Object.keys(details);if(!races.length)return'';
 return `<div class="creation-wide selected-race-details-switcher" data-selected-race-details-shell>${races.map(race=>`<div data-selected-race-details="${escapeHtml(race)}" ${race===selectedRace?'':'hidden'}>${renderSelectedRaceDetails(race,null,data)}</div>`).join('')}<section class="section stat-panel race-reference-panel" data-selected-race-details-empty ${selectedRace?'hidden':''}><div class="stat-panel-head"><div><div class="kicker">Selected Race</div><h3>Abilities &amp; Drawbacks</h3></div><span class="race-reference-badge">Choose a race</span></div><p class="muted race-reference-summary">Choose an unlocked race above to view its racial abilities, traits, drawbacks, and tradeoffs before binding the Vessel.</p></section></div>`;
}
