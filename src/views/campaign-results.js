...
import { itemTypeTag, equipmentSlotLabel, formatListedStats, weaponCompatibilityText } from '../player-facing.js';
import { makeLegacyEquipmentItem, legacyModifierLabel } from '../legacy-lender.js';
...
export function renderCampaignResults({ settlement, equipmentCatalog = null, message = '' }) {
  ...
  return shell(`
    ...
    ${(settlement.lender?.candidates||[]).length?`<section class="panel section"><h2>Choose One Item for Mara's Lender</h2><p class="muted">This successful return may permanently register exactly one eligible item for this Vessel's lender collection. Future borrowed copies gain the lender-exclusive <strong>Legacy</strong> modifier.</p><div class="lender-grid">${settlement.lender.candidates.map(id=>{const item=(equipmentCatalog?.equipment||[]).find(x=>x.id===id);const legacy=item?makeLegacyEquipmentItem(item):null;const stats=legacy?formatListedStats(legacy):'';const compat=item?.itemType==='Weapon'?weaponCompatibilityText(item,settlement.baseClass||null):'';return `<button class="lender-item ${settlement.lender.selectedItemId===id?'selected':''}" data-action="campaign-lender-select" data-item="${escapeHtml(id)}"><strong>${escapeHtml(item?.name||id)}</strong>${item?`<span class="legacy-modifier-badge">Legacy when borrowed</span><span class="item-type-tag">${escapeHtml(itemTypeTag(item))}</span><small>${escapeHtml(equipmentSlotLabel(item.slot))}</small>`:''}${stats?`<small>${escapeHtml(stats)}</small>`:''}${legacy?`<small>${escapeHtml(legacyModifierLabel())}</small>`:''}${compat?`<small class="compatibility-note">${escapeHtml(compat)}</small>`:''}<small>${settlement.lender.selectedItemId===id?'Selected':'Register this item'}</small></button>`}).join('')}</div></section>`:''}
