...
export function awardCurrentForestCombatRewards(slot,{progression,rng=Math.random,now=new Date().toISOString()}={}){
  ...
  const exp=defeated.reduce((n,a)=>n+Math.max(0,Number(a.expReward||0)),0),rawOnyx=defeated.reduce((n,a)=>n+Math.max(0,Number(a.onyxReward||0)),0);const deployedAdventurers=Object.keys(run.adventurers||{}).length;const multiplier=adventurerOnyxMultiplier(deployedAdventurers,progression);let regionalRewardMultiplier=1;if(run.expedition?.regionId==='ruined-vampiric-plains'){const value=Math.max(0,Math.min(100,Number(run.expedition?.bloodMoon||0))),tier=(run.expedition?.regionalMechanic?.tiers||[]).find(t=>value>=Number(t.min||0)&&value<=Number(t.max||100));regionalRewardMultiplier=1+Math.max(0,Number(tier?.rewardBonusPct||0))/100;}const onyx=Math.round(rawOnyx*multiplier*regionalRewardMultiplier);const chronicleProgress=chronicleFromRawOnyx(rawOnyx*regionalRewardMultiplier,progression);
  ...
  const reward={outcome:combat.outcome,defeatedEnemyIds:defeated.map(a=>a.id),exp,rawOnyx,onyxMultiplier:multiplier,regionalRewardMultiplier,onyxAwarded:onyx,chronicleProgress,deployedAdventurers,experienceAwards:clone(expResult.awards),awardedAt:now};
