...
function executeDamageAbility(next,combat,actor,ability,{difficulty,rng}){
  ...
  for(const target of targets){
    ...
    if(targetWasHit&&Number(ability.greedStealShieldPct||0)>0){const before=Math.max(0,Number(target.resources?.shield||0)),stolen=Math.max(0,Math.round(before*Number(ability.greedStealShieldPct)/100));target.resources.shield=Math.max(0,before-stolen);actor.resources.shield=Math.max(0,Number(actor.resources?.shield||0)+stolen);outcomes.push({type:'shield-steal',targetId:target.id,amount:stolen});}
