...
function currentPlainsRewardBonus(run){if(run?.expedition?.regionId!=='ruined-vampiric-plains')return 0;const value=Math.max(0,Math.min(100,Number(run.expedition?.bloodMoon||0)));const tier=(run.expedition?.regionalMechanic?.tiers||[]).find(t=>value>=Number(t.min||0)&&value<=Number(t.max||100));return Math.max(0,Number(tier?.rewardBonusPct||0));}
function applyOutcomeEffects(next,participantId,effect,{equipmentCatalog,forestCrafting,progression}={}){
  ...
  const rewardBonus=currentPlainsRewardBonus(run);const baseOnyx=Math.max(0,Number(effect.onyx||0)),onyx=Math.max(0,Math.round(baseOnyx*(1+rewardBonus/100))); if(onyx){run.rewards.carriedOnyx=Number(run.rewards.carriedOnyx||0)+onyx;run.rewards.chronicleProgress=Number(run.rewards.chronicleProgress||0)+onyx/25;applied.onyx=onyx;applied.baseOnyx=baseOnyx;applied.bloodMoonRewardBonusPct=rewardBonus;applied.chronicleProgress=onyx/25;}
  ...
  if(effect.materialId){const baseQ=Math.max(0,Number(effect.materialQuantity||1));const q=run?.expedition?.regionId==='ruined-vampiric-plains'?Math.max(1,Math.round(baseQ*(1+rewardBonus/100))):baseQ;addInventoryMaterial(run,effect.materialId,q,forestCrafting);applied.material={id:effect.materialId,quantity:q,baseQuantity:baseQ};}
  if(Array.isArray(effect.materials)){const granted=[];for(const entry of effect.materials){const id=String(entry?.id||entry?.materialId||'').trim();const baseQ=Math.max(0,Number(entry?.quantity??entry?.materialQuantity??1));if(!id||baseQ<=0)continue;const q=run?.expedition?.regionId==='ruined-vampiric-plains'?Math.max(1,Math.round(baseQ*(1+rewardBonus/100))):baseQ;addInventoryMaterial(run,id,q,forestCrafting);granted.push({id,quantity:q,baseQuantity:baseQ});}if(granted.length)applied.materials=granted;}
  ...
  if(run?.expedition?.regionId==='ruined-vampiric-plains'){
    const ex=run.expedition,metrics=(run.regionalMetrics||(run.regionalMetrics={})).plains||((run.regionalMetrics.plains)={});
    if(Number(effect.bloodMoonDelta||0)){const before=Number(ex.bloodMoon||0);ex.bloodMoon=clamp(before+Number(effect.bloodMoonDelta||0),0,100);ex.bloodMoonMax=Math.max(Number(ex.bloodMoonMax||0),ex.bloodMoon);metrics.bloodMoonMax=Math.max(Number(metrics.bloodMoonMax||0),ex.bloodMoon);applied.bloodMoon={before,after:ex.bloodMoon,delta:ex.bloodMoon-before};}
    if(Number(effect.sanguineFontLifestealPct||0)>0){ex.sanguineFontLifestealPct=Math.max(Number(ex.sanguineFontLifestealPct||0),Number(effect.sanguineFontLifestealPct));applied.sanguineFontLifestealPct=ex.sanguineFontLifestealPct;metrics.hpSacrificeEvents=Number(metrics.hpSacrificeEvents||0)+1;}
    else if(Number(effect.bloodMoonDelta||0)>=5)metrics.hpSacrificeEvents=Number(metrics.hpSacrificeEvents||0)+1;
  }
  if(run?.expedition?.regionId==='necropolis'){
    const ex=run.expedition,metrics=(run.regionalMetrics||(run.regionalMetrics={})).necropolis||((run.regionalMetrics.necropolis)={});
    if(Number(effect.necropolisMirrorGlimpse||0)>0){const q=Math.max(0,Math.trunc(Number(effect.necropolisMirrorGlimpse||0)));ex.mirrorGlimpses=Number(ex.mirrorGlimpses||0)+q;metrics.mirrorGlimpses=Number(metrics.mirrorGlimpses||0)+q;applied.necropolisMirrorGlimpse=q;}
    if(Number(effect.necropolisCultTrail||0)!==0){const q=Number(effect.necropolisCultTrail||0);ex.cultTrail=Math.max(0,Number(ex.cultTrail||0)+q);applied.necropolisCultTrail=q;}
  }
  if(run?.expedition?.regionId==='that-dragons-dungeon'){
    const ex=run.expedition,metrics=(run.regionalMetrics||(run.regionalMetrics={})).dragon||((run.regionalMetrics.dragon)={});
    if(Number(effect.dragonTreasureTaken||0)>0){const q=Math.max(0,Math.trunc(Number(effect.dragonTreasureTaken||0)));ex.dragonTreasuresTaken=Number(ex.dragonTreasuresTaken||0)+q;metrics.treasuresTaken=Math.max(Number(metrics.treasuresTaken||0),Number(ex.dragonTreasuresTaken||0));applied.dragonTreasureTaken=q;}
    if(effect.dragonCurse?.id){ex.dragonCurses=Array.isArray(ex.dragonCurses)?ex.dragonCurses:[];const curse={id:String(effect.dragonCurse.id),name:String(effect.dragonCurse.name||'Hoard Curse'),modifiers:clone(effect.dragonCurse.modifiers||{})};if(!ex.dragonCurses.some(x=>x.id===curse.id)){ex.dragonCurses.push(curse);applied.dragonCurse=clone(curse);}metrics.cursesSurvived=Math.max(Number(metrics.cursesSurvived||0),ex.dragonCurses.length);}
  }

  return applied;
}
function applyTowerFall(run,effect,{equipmentCatalog,progression}={}){const depths=Math.max(0,Math.trunc(Number(effect?.fallDepths||0))),pct=Math.max(0,Number(effect?.fallPartyHpLossPct||0));if(!depths&&!pct)return null;const ex=run.expedition;if(ex?.regionId!=='heavenly-tower')return null;const beforeDepth=Number(ex.depth||1),afterDepth=Math.max(1,beforeDepth-depths),members=[];for(const id of ['vessel',...Object.keys(run.adventurers||{})]){const p=participantState(run,id);if(!p)continue;const max=maxHpForParticipant(run,id,equipmentCatalog,progression),before=Number.isFinite(Number(p.state.currentHp))?Number(p.state.currentHp):max;if(before<=0)continue;const loss=Math.round(max*pct/100),after=Math.max(1,before-loss);p.state.currentHp=after;members.push({id,name:p.name,before,after,maxHp:max,hpLost:before-after});}ex.depth=afterDepth;ex.fallRecoveryPending=true;run.regionalMetrics=run.regionalMetrics||{};run.regionalMetrics.tower=run.regionalMetrics.tower||{};run.regionalMetrics.tower.falls=Number(run.regionalMetrics.tower.falls||0)+1;return{type:'tower-fall',depthBefore:beforeDepth,depthAfter:afterDepth,depthsLost:beforeDepth-afterDepth,maxHpLossPct:pct,party:members};}
