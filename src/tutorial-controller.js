...
function baseState(){return {starter:{resolved:false,resolution:null,rewardGranted:false,resolvedAt:null},statuses:{},tokenWallet:{keptImpression3OrLess:0},contextualSeen:[]};}
export function normalizeTutorialState(account){const next=clone(isRecord(account)?account:{});const current=isRecord(next.tutorials)?next.tutorials:{};const base=baseState();const starter=isRecord(current.starter)?current.starter:{};const wallet=isRecord(current.tokenWallet)?current.tokenWallet:{};const rawStatuses=isRecord(current.statuses)?current.statuses:{};const statuses={};for(const [id,status] of Object.entries(rawStatuses))statuses[id]=TUTORIAL_STATES.includes(status)?status:'never-seen';const token=Number(wallet.keptImpression3OrLess);next.tutorials={...base,...current,starter:{...base.starter,...starter,resolved:Boolean(starter.resolved),resolution:['completed','skipped'].includes(starter.resolution)?starter.resolution:null,rewardGranted:Boolean(starter.rewardGranted),resolvedAt:typeof starter.resolvedAt==='string'?starter.resolvedAt:null},statuses,tokenWallet:{...base.tokenWallet,...wallet,keptImpression3OrLess:Number.isFinite(token)?Math.min(2,Math.max(0,Math.trunc(token))):0},contextualSeen:stringList(current.contextualSeen)};return next;}
...
export function resolveStarterTutorial(account,resolution='completed'){
  ...
  let next=normalizeTutorialState(account);if(next.tutorials.starter.resolved)return {ok:true,account:next,granted:0,alreadyResolved:true};
  next.tutorials.starter={resolved:true,resolution,rewardGranted:true,resolvedAt:new Date().toISOString()};
  ...
  return {ok:true,account:next,granted:2-before,alreadyResolved:false};
}
