...
export function getMantleAvailability({ slot, account, subclassesForBase = [] }) {
  if (!isProgressionFeatureUnlocked(account, PROGRESSION_FEATURES.MANTLE)) {
    return { available: false, reason: 'The Mantle Room has not opened yet.', unlockedChoices: [] };
  }
  ...
  const allowed = new Set(subclassesForBase);
  const unlockedChoices = (account?.unlocks?.subclasses || []).filter(name => allowed.has(name));
  return { available: true, reason: '', unlockedChoices };
}

export function selectMantle({ slot, account, subclass, subclassesForBase = [] }) {
  const state = getMantleAvailability({ slot, account, subclassesForBase });
  if (!state.available) return { ok: false, error: state.reason };
  if (subclass !== null && !state.unlockedChoices.includes(subclass)) return { ok: false, error: 'That subclass Mantle is not available to this Vessel.' };
  const next = clone(slot);
  next.character = { ...(next.character || {}), subclass: subclass || null };
