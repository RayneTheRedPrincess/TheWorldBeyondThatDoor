...
function token(runId,depth,ordinal,rng){return `${runId}-necro-d${depth}-c${ordinal}-${Math.floor(unit(rng)*0xffffff).toString(36).padStart(5,'0')}`;}
...
export function createNecropolisEventCards({runId,depth,necropolisEvents,expedition,rng=Math.random}={}){
 const events=necropolisEvents?.events||[],used=new Set(expedition?.usedEventIds||[]),count=3,tier=tierForDepth(depth);let chosen=[];
 if(depth===1&&!(expedition?.history||[]).length)chosen=events.filter(e=>String(e.id).startsWith('necro-intro-')).slice(0,3);
 else {const pool=events.filter(e=>!String(e.id).startsWith('necro-intro-')&&!used.has(e.id)&&Number(e.minTier||tier)<=tier&&Number(e.maxTier||tier)>=tier);while(chosen.length<count){const avail=pool.filter(e=>!chosen.includes(e));if(!avail.length)break;const x=weightedPick(avail,rng);if(x)chosen.push(x);}}
 if(chosen.length!==count)throw new Error(`Necropolis event authority cannot supply ${count} unique cards at Depth ${depth}.`);
