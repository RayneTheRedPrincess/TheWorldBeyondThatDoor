...
function tierForDepth(depth){if(depth<=5)return 1;if(depth<=9)return 2;if(depth<=15)return 3;if(depth<=19)return 4;if(depth<=25)return 5;return 6;}
...
function token(runId,depth,ordinal,rng){return `${runId}-dragon-d${depth}-c${ordinal}-${Math.floor(unit(rng)*0xffffff).toString(36).padStart(5,'0')}`;}
...
export function createDragonEventCards({runId,depth,dragonEvents,expedition,rng=Math.random}={}){
 const events=dragonEvents?.events||[],used=new Set(expedition?.usedEventIds||[]),count=3,tier=tierForDepth(depth);let chosen=[];
 if(depth===1&&!(expedition?.history||[]).length)chosen=events.filter(e=>String(e.id).startsWith('dragon-intro-')).slice(0,3);
 else {const pool=events.filter(e=>!String(e.id).startsWith('dragon-intro-')&&!used.has(e.id)&&Number(e.minTier||tier)<=tier&&Number(e.maxTier||tier)>=tier);while(chosen.length<count){const avail=pool.filter(e=>!chosen.includes(e));if(!avail.length)break;const x=weightedPick(avail,rng);if(x)chosen.push(x);}}
 if(chosen.length!==count)throw new Error(`That Dragon’s Dungeon event authority cannot supply ${count} unique cards at Depth ${depth}.`);
